...
void Screens::drawWaiting(ScreenState& screen) {
    ...
    if (_selfId != 0 && _selfId == _hostId) {
        ...
        std::string dlabel = std::string("Difficulty: ") + diffNames[std::min(2, (int)_lobbyDifficulty)];
        titleCentered(dlabel.c_str(), diffY, baseFont, RAYWHITE);

        int btnW = (int)(w * 0.06f);
        int btnH = (int)(h * 0.06f);
        int bx = (w - (btnW * 2 + (int)(w*0.02f))) / 2;
        Rectangle dMinus{(float)bx, (float)(diffY + (int)(h*0.04f)), (float)btnW, (float)btnH};
        Rectangle dPlus{(float)(bx + btnW + (int)(w*0.02f)), (float)(diffY + (int)(h*0.04f)), (float)btnW, (float)btnH};
        bool changed = false;
        if (button(dMinus, "-", baseFont, BLACK, LIGHTGRAY, GRAY)) { if (_lobbyDifficulty > 0) { _lobbyDifficulty--; changed = true; } }
        if (button(dPlus, "+", baseFont, BLACK, LIGHTGRAY, GRAY)) { if (_lobbyDifficulty < 2) { _lobbyDifficulty++; changed = true; } }

        // Base lives selector
        int livesY = diffY + (int)(h * 0.12f);
        std::string llabel = std::string("Base lives: ") + std::to_string((int)_lobbyBaseLives) + " (max 6)";
        titleCentered(llabel.c_str(), livesY, baseFont, RAYWHITE);
        Rectangle lMinus{(float)bx, (float)(livesY + (int)(h*0.04f)), (float)btnW, (float)btnH};
        Rectangle lPlus{(float)(bx + btnW + (int)(w*0.02f)), (float)(livesY + (int)(h*0.04f)), (float)btnW, (float)btnH};
        if (button(lMinus, "-", baseFont, BLACK, LIGHTGRAY, GRAY)) { if (_lobbyBaseLives > 1) { _lobbyBaseLives--; changed = true; } }
        if (button(lPlus, "+", baseFont, BLACK, LIGHTGRAY, GRAY)) { if (_lobbyBaseLives < 6) { _lobbyBaseLives++; changed = true; } }
        ...
        Rectangle startBtn{(float)((w - startW)/2), (float)startY, (float)startW, (float)startH};
        Color bg = playerCount >= 2 ? (Color){120, 200, 120, 255} : (Color){80, 120, 80, 255};
        Color hg = playerCount >= 2 ? (Color){150, 230, 150, 255} : (Color){90, 140, 90, 255};
        if (button(startBtn, "Start", baseFont, BLACK, bg, hg)) {
            if (playerCount >= 2) sendStartMatch();
        }
    } else {
